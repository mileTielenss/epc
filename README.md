...
- **Ruimtebalk in de header** (op Details en Foto's, ook in het camerascherm): kies in welke ruimte je staat; alles wat je toevoegt krijgt die ruimte als label. Ruimtes zijn altijd binnen het beschermd volume: op Details is er geen "Buiten" en is altijd een ruimte gekozen; alleen in de foto-context (Foto's en camerascherm) bestaat de chip "Buiten" voor gevelfoto's e.d. Elke woning start met Living, Keuken, Badkamer, WC, Berging en Slaapkamer 1; "+ Ruimte" opent sneltoetsen met autonummering (Slaapkamer → Slaapkamer 2...), × verwijdert de gekozen ruimte (items verliezen enkel hun label). Per ruimte cycle je de ventilatie met één knop: geen → natuurlijk → mechanisch → ander (met eigen beschrijving).
