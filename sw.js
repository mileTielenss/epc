...
self.addEventListener('fetch', e => {
  ...
  /* vreemde oorsprong (NAS, Nominatim): helemaal niet aanraken. Zo blijft de
     échte browserfout zichtbaar (certificaat, CORS) in plaats van een
     algemene "Load failed" uit deze handler (§4) */
  if (new URL(e.request.url, self.location.origin).origin !== self.location.origin) return;
  e.respondWith(
